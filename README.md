...
- **Can you trust the judge?** `ragcheck validate-judge labels.jsonl --metric faithfulness` measures LLM-judge vs. human agreement (Cohen's kappa + confusion matrix) *before* you trust judged metrics. On our own labels it caught a real miscalibration: the default pass threshold scored kappa=0.40; the corrected threshold scores kappa=1.00. Reports display "validated at kappa=0.XX" - or a visible "not validated" flag.
...
| Built-in judge validation vs. human labels (Cohen's kappa) | ✅ | ❌ | ❌ | ❌ |
...
Early hypothesis worth noticing: agentic RAG refuses unanswerables best but answers paraphrases least consistently, at 1.75x the cost - exactly the trade-offs single-metric evals miss. Methodology, frontier chart, and honest limitations: [benchmarks/](benchmarks/README.md).

## Design decisions

- **Judge validation is a feature, not a footnote.** Judged metrics are untrustworthy by default; the kappa workflow makes trust measurable, and reports flag unvalidated judges.
...
| Judge validation (`validate-judge`, Cohen's kappa) | ✅ |
